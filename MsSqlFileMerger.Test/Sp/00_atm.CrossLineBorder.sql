if exists (select 1 from sysobjects where id = object_id('atm.CrossLineBorder'   )and type in ('P','PC'))  drop procedure atm.CrossLineBorder
go
...
as
begin
    -- обработка пересечения линии помеченной как граница зоны
	...
	-- ищем активный судопроход по ВСЕМ ЗОНАМ кроме текущей
	Select   
    ...
    if @crossType = 'out' 
	begin -- внешняя линия значит ВХОД В ЗОНУ
        if not @prevPassGuid is null
 	    begin -- есть судопроход
		  -- надо закрыть старый судопроход
            ...
            select
                  ...
                  , @prevVtsCode           -- ActVtsCode              nvarchar( 2)     null -- код СУДС
                  , @prevVtsZoneCode          -- ActZoneCode             nvarchar( 2)     null -- код зоны
                  , @prevPassDirectionCode -- ActPassDirectionCode    nvarchar(10)     null -- направление движения
		          ...
		          Set @actPassDirectionCode = @prevPassDirectionCode ---atm.fnGetDirectionByInPos(@actPositionCode,@crossType);  -- по позиции, типу позиции вычисляем откуда пришло судно- "с севера"/"с юга"

         -- ищем активный судопроход или черновик в текущей зоне
	     Select   
	          @prevPassGuid           = PassGuid
            , @prevReceiptTypeCode    = ReceiptTypeCode
            , @prevVtsCode            = VtsCode
